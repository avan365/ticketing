  ...
  if (!import.meta.env.DEV) {
    try {
      ...
      if (!response.ok) {
        ...
        return {
          success: false,
          error: error.error || "Failed to update ticket",
        };
      }

      // Fetch updated order to get details
      const ticketResponse = await fetch(
        `${API_URL}/api/orders?ticketId=${ticketId}`
      );
...
export async function findTicket(
  orderNumber: string,
  ticketId: string
): Promise<IndividualTicket | null> {
  const orders = await getAllOrders();
  const normalizedOrderNumber = orderNumber.trim().toUpperCase();
  const normalizedTicketId = ticketId.trim().toUpperCase();

  const order = orders.find(
    (o) => o.orderNumber.toUpperCase() === normalizedOrderNumber
  );
...
export async function getOrderStats() {
  ...
  const totalRevenue = orders
    ...
    .reduce((sum, o) => {
      ...
      return (
        sum +
        o.tickets.reduce((ticketSum, t) => ticketSum + t.price * t.quantity, 0)
      );
    }, 0);
